...
def normalise_input(string):
    ...
    'help'
    """
    return remove_spaces(remove_punct(string)).lower()
    
def display_room(string):
...
def exit_leads_to(exits, direction):
    ...
    for exit in exits:
        if exit == direction:
            return rooms[exits[exit]]['name']  

def print_menu_line(direction, leads_to):
    """This function prints a line of a menu of exits. It takes a direction (the
    name of an exit) and the name of the room into which it leads (leads_to),
    leadsand should print a menu line in the following format:
...
def menu(exits):
    ...
    while True:
        ...
        playerinput = input()
        # Normalise the input
        norminput = normalise_input(playerinput)
        # Check if the input makes sense (is valid exit)
        if is_valid_exit(exits, norminput):
